...
"""
# ---------------------------------------------------------------------------
# WASM / JupyterLite variant
# ---------------------------------------------------------------------------
# This is a lightweight fork of document_similarity.py adapted to run fully in
# the browser on the Pyodide (WebAssembly) kernel used by JupyterLite. The
# original tool depended on three packages that are NOT available in Pyodide:
#   * atap_corpus / atap_corpus_loader -> pull in spaCy + pyarrow (compiled)
#   * gensim                           -> compiled Cython extensions
#   * swifter                          -> dask/ray parallelism (no threads in WASM)
# None of them were used for anything that can't be done with the stdlib + the
# packages Pyodide already ships, so they have been replaced in place:
#   * gensim.utils.tokenize -> a regex tokenizer identical to gensim's default
#   * df.swifter.apply(...) -> plain df.apply(...) (single-threaded in WASM anyway)
#   * Corpus loader UI      -> a small ipywidgets file uploader (see build_corpus_*)
# The similarity/MinHash/visualisation logic below is unchanged.
# ---------------------------------------------------------------------------
...
# NLTK: natural language processing tools for working with language/text data
import nltk
...
# the browser). There is no server filesystem there, so downloads and the export
# format behave differently.
IS_PYODIDE = (sys.platform == "emscripten") or ("pyodide" in sys.modules)


# ---------------------------------------------------------------------------
# Drop-in replacement for gensim.utils.tokenize.
# This mirrors gensim's default PAT_ALPHABETIC behaviour: it yields runs of
# word characters that do not start with a digit, so tokenisation (and the
# resulting Jaccard scores) match the original tool.
# ---------------------------------------------------------------------------
PAT_ALPHABETIC = re.compile(r'(((?![\d])\w)+)', re.UNICODE)


def tokenize(text):
    """Yield alphabetic tokens from ``text`` (gensim-compatible)."""
...
class DownloadFileLink(HTML):
    ...
    non-existent URL ("File wasn't available on site"). Instead the file's bytes
    are embedded as a base64 ``data:`` URI. This is a plain href (no
    ...
    On a Jupyter server (Binder) the href instead points at the file served by
    the server, which streams it on click without loading it into memory.
    """
...
class DocumentSimilarity:
    ...
    def set_text_df(self, corpus_df: pd.DataFrame):
        """
        Build the internal text DataFrame from a plain pandas DataFrame.

        In the original (BinderHub) tool this accepted an atap_corpus ``Corpus``
        object and called ``corpus.to_dataframe()``. In the WASM build we skip
        that dependency and accept a DataFrame directly. To stay backward
        compatible, an object exposing ``.to_dataframe()`` is still accepted.

        The DataFrame is expected to have a text column (``text`` or the
        original ``document_``) and optionally a ``text_name``/``filename``
        column.
        """
        if hasattr(corpus_df, 'to_dataframe'):
            corpus_df = corpus_df.to_dataframe()

        new_text_df = pd.DataFrame(columns=['text'], dtype=str)
        if 'text' in corpus_df.columns:
            new_text_df['text'] = corpus_df['text'].copy()
        elif 'document_' in corpus_df.columns:
            new_text_df['text'] = corpus_df['document_'].copy()
        else:
            raise ValueError("Input must have a 'text' (or 'document_') column.")
        if 'text_name' in corpus_df.columns:
            ...
            display(HTML('<div style="background:#fff3cd;border:1px solid #ffeeba;'
                         'padding:8px;border-radius:4px;">No "text_name" column found; '
                         'using the row index as the document name.</div>'))
    ...
    def calculate_similarity(self,
        ...
        try:
            ...
            self.text_df['word_count'] = self.text_df.apply(
            ...
            self.text_df['matched_list'] = self.text_df.apply(
            ...
            intermediate_df = self.text_df[['matched_list',
                                            ...
                                            'text_name',
                                            'jaccards']].copy()
            intermediate_df['listlen'] = intermediate_df.apply(
                lambda x: len(x.matched_list), axis=1)

            intermediate_df = intermediate_df[intermediate_df.listlen > 0]
            intermediate_df['text_id_duped'] = intermediate_df.apply(
    ...
    def save_to_csv(self,
        ...
        # into len(df) chunks -- one row per chunk -- and appended row by row,
        # which is O(n) file operations and extremely slow on large corpora.)
        df.to_csv(out_dir + file_name, index=True)

    def save_results(self, df: pd.DataFrame, base_name: str):
        """
        Save the deduplication results.

        Under JupyterLite this is always a single CSV with the ``text_name`` and
        ``text`` columns -- one file, light to build and download in the browser
        (writing thousands of individual files and zipping them is too slow and
        memory-heavy in the browser). The Binder version can instead export a
        zip of .txt files to match a file-based upload.
    ...
    def save_to_zip(self,
        ...
        shutil.rmtree('./output/saved_files', ignore_errors=True)
...
# ===========================================================================
# Lightweight in-browser corpus loader (replacement for atap_corpus_loader)
# ===========================================================================
# The original notebook used `from atap_corpus_loader import CorpusLoader`,
# which depends on spaCy + pyarrow and cannot run on Pyodide. This is a small
# ipywidgets-based uploader that reads .txt / .csv / .xlsx / .zip files the
# user selects in the browser and produces a pandas DataFrame with two
# columns: `text` and `text_name`. Feed the result straight into
# `DocumentSimilarity.set_text_df(...)`.

SUPPORTED_EXTENSIONS = ('.txt', '.csv', '.xlsx', '.zip')


def _records_from_tabular(df: pd.DataFrame, source_name: str):
    """Extract (text, text_name) records from a CSV/XLSX DataFrame.

    Looks for a text column named (case-insensitively) 'text', otherwise falls
    back to the first column. Uses a 'text_name'/'filename'/'title' column for
    names if present, else generates names from the source file + row number.
    """
    cols = {c.lower(): c for c in df.columns}
    text_col = cols.get('text', df.columns[0])
    name_col = cols.get('text_name') or cols.get('filename') or cols.get('title')
    ...
    for i, row in df.iterrows():
        ...
        if name_col is not None and not pd.isna(row[name_col]):
            ...
            name = f'{source_name}_row{i}'
        records.append({'text': str(text), 'text_name': name})
    return records


def _records_from_bytes(filename: str, data: bytes):
    """Turn the raw bytes of one uploaded file into a list of text records."""
    name = os.path.basename(filename)
    lower = name.lower()
    records = []

    if lower.endswith('.txt'):
        records.append({'text': data.decode('utf-8', errors='replace'),
                        'text_name': name})
    elif lower.endswith('.csv'):
        df = pd.read_csv(io.BytesIO(data))
        records.extend(_records_from_tabular(df, os.path.splitext(name)[0]))
        ...
        with zipfile.ZipFile(io.BytesIO(data)) as zf:
            for member in zf.namelist():
                if member.endswith('/') or os.path.basename(member).startswith('.'):
                    continue
                if member.lower().endswith(SUPPORTED_EXTENSIONS) and not member.lower().endswith('.zip'):
                    records.extend(_records_from_bytes(member, zf.read(member)))
    return records


class FileUploaderWidget:
    """A minimal browser file uploader that builds a corpus DataFrame.

    Usage in the notebook::

        loader = FileUploaderWidget()
        loader            # displays the upload UI
        # ... user selects files and clicks "Build corpus" ...
    ...
    def __init__(self):
        ...
        self._build_button = widgets.Button(
            description='Build corpus',
            button_style='success',
            layout=Layout(width='160px', margin='0 0 0 10px'),
            style=dict(font_weight='bold'),
        )
        self._out = widgets.Output()
        self._build_button.on_click(self._on_build)

    def _iter_uploaded(self):
        """Yield (filename, content_bytes) across ipywidgets 7.x and 8.x APIs."""
        value = self._uploader.value
        # ipywidgets >= 8: value is a tuple of dicts with 'name'/'content'
        ...
        # ipywidgets 7: value is a dict keyed by filename
            ...
            for fname, item in value.items():
                yield fname, bytes(item['content'])

    def _on_build(self, _):
        with self._out:
            clear_output()
            records = []
            uploaded = list(self._iter_uploaded())
            if not uploaded:
                print('No files selected. Use "Select files" first, then click "Build corpus".')
                return
            for fname, content in uploaded:
                if not fname.lower().endswith(SUPPORTED_EXTENSIONS):
                    ...
                    continue
                try:
                    records.extend(_records_from_bytes(fname, content))
                except Exception as exc:
                    print(f'Could not read {fname}: {exc}')
            if not records:
                print('No readable documents found in the selected files.')
                self.corpus_df = None
                return
            self.corpus_df = pd.DataFrame(records, columns=['text', 'text_name'])
            print(f'Corpus built: {len(self.corpus_df)} document(s) from {len(uploaded)} file(s).')
            print('Now run the next cell to load this corpus into the tool.')

    def get_dataframe(self):
        """Return the built corpus DataFrame (or None if not built yet)."""
        return self.corpus_df

    def _ipython_display_(self):
        display(widgets.VBox([
            widgets.HTML('<b>Upload your documents</b> (.txt, .csv, .xlsx or a .zip of these), '
                         'then click <b>Build corpus</b>.'),
            widgets.HBox([self._uploader, self._build_button]),
            self._out,
        ]))
